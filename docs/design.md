...
15 presets, mapped 1:1 to wizard selections.
...
Application order: `base → typescript → python → react → nextjs → fastapi → express → aws → azure → gcp → cdk → cloudformation → terraform → bicep → claude-code`
