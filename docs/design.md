...
| 7 | AI Agent tools | Multi-select | Claude Code / Codex CLI / Gemini CLI / Amazon Q Developer / GitHub Copilot / Cline (future: Cursor) |
...
| 6 | Agent | 複数選択可 | `claude-code`, `codex`, `gemini`, `amazon-q`, `copilot`, `cline` |
...
Application order: `base → typescript → python → react → nextjs → fastapi → express → aws → azure → gcp → cdk → cloudformation → terraform → bicep → claude-code → codex → gemini → amazon-q → copilot → cline`
